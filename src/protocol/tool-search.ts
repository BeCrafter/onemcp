...
export const INVOKE_TOOL_DEFINITION = {
  name: 'onemcp__invoke',
  description: `Execute a tool previously discovered via onemcp__search.

WHEN TO USE:
- Your MCP client refuses to call tool names that were not in tools/list — route the call through this wrapper instead.
- You want a uniform call shape regardless of which underlying service hosts the tool.

HOW TO USE:
- Set "namespacedName" to the exact string returned by onemcp__search (e.g. "prompx___action").
- Put the underlying tool's own arguments into the "arguments" object — its inputSchema (returned by search) is the source of truth.
- The original tool's description, MANDATORY/Must-Check markers, and inputSchema all still apply at call time. Re-read them from the search result.

NOTE: Direct calls by namespacedName are still supported for clients that allow them; this wrapper is the portable path.`,
  inputSchema: {
    ...
    properties: {
      namespacedName: {
        type: 'string',
        description:
          'Exact namespacedName returned by onemcp__search, e.g. "prompx___action" or "skill-mcp___skill_list".',
      },
      arguments: {
        type: 'object',
        description:
          "Arguments object matching the underlying tool's inputSchema (as returned in the search result). May be empty if the tool takes no arguments.",
...
/**
 * Static preamble for the dynamic search description.
 * Re-exported for `buildSmartDiscoverySearchDescription` so the static how-to
 * guidance always leads, with PROACTIVE TRIGGERS / SERVICE INDEX appended.
 */
export const SEARCH_DESCRIPTION_PREAMBLE = SEARCH_TOOL_DEFINITION.description;
