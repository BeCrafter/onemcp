...
const killServer = () => spawnSync('tmux', ['-L', SOCKET, 'kill-server'], { encoding: 'utf8', env: TMUX_ENV });
...
async function t1ListLayout() {
  ...
  check('34 行下 16 个服务全部渲染', names.every((n) => lines34.some((l) => nameInRow(l) === n)));
  check(
    '每个服务恰好占一行（无换行续行）',
    names.every((n) => lines34.filter((l) => nameInRow(l) === n).length === 1)
  );
  check('每行不超过终端宽度', lines34.every((l) => l.length <= 100));
  ...
  check('画面不超出终端高度（写入行数 ≤ 行数上限）', lines34.length <= 34, `写入 ${lines34.length} 行`);
...
async function t2NarrowTerminal() {
  ...
  check('仍每服务一行', names.every((n) => lines.filter((l) => nameInRow(l) === n).length === 1));
  check('丢弃 tags 列以换取端点宽度', !text.includes('[tag0]'));
  check('端点仍可读（省略号截断）', text.includes('https://svc-0.example.c…') || text.includes('…'));
  check('每行不超过 60 列', lines.every((l) => l.length <= 60));
}
...
async function t4OverwriteConfirm() {
  ...
  check('确认前不覆盖（tags 完好）', readConfig(configDir).mcpServers['existing'].tags.join() === 'keep,this');

  await sendText('n');
  await waitFor(() => capture().includes('Cancelled'));
  check('n 取消后配置未变', readConfig(configDir).mcpServers['existing'].tags.join() === 'keep,this');
}
...
async function t8ToolsView() {
  ...
  const filtered = await waitFor(() => capture().includes('Search: big') && capture().includes('1/4'));
...
async function t9ResultActions() {
  ...
  check(
    ...
    await waitFor(() => capture().includes('Copied') || capture().includes('No clipboard utility'), 5000)
  );

  await sendText('f');
  check('f 切到全宽（工具列表隐藏）', await waitFor(() => !capture().includes('mock__') || capture().includes('OUTPUT'), 4000));
}
...
async function t11Cjk() {
  ...
  check('[中文标签] 出现在两个服务所在的行', tagLines('[中文标签]') === 2, `出现在 ${tagLines('[中文标签]')} 行`);
  check('[宽字符] 出现在一个服务所在的行', tagLines('[宽字符]') === 1, `出现在 ${tagLines('[宽字符]')} 行`);
  check(
    '每个服务恰好一行（宽字符未导致折行）',
  ...
  await sendKey('C-r');
  check('中文参数值运行后原样回显', await waitFor(() => capture().includes('echo: "你好世界"'), 10_000));
}
...
async function t12ConfigPathAndSelfWriteNotice() {
  ...
  const created = await waitFor(() => capture().includes("Service 'e2e-selfwrite' created successfully"), 6000);
...
async function t13SaveOutput() {
  ...
  await sendKey('C-o');
  check('Ctrl+O 提示已保存并给出路径', await waitFor(() => capture().includes('Saved full output:'), 5000));
  check(
...
function assertFreshBuild() {
  ...
  if (stale.length > 0) {
    process.stderr.write(
      `dist 比源码旧（例如 ${stale[0]}）—— 请先运行 npm run build 再验证 TUI\n`
    );
    process.exit(1);
  }
}
...
async function main() {
  ...
  for (const scenario of scenarios) {
    try {
      ...
      check(`${scenario.name} 未抛异常`, false, error instanceof Error ? error.message : String(error));
    } finally {
      if (!KEEP) killServer();
    }
  }
