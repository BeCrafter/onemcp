...
- 🔍 **智能工具发现** - 默认关闭，启用后只暴露搜索工具，AI Agent 按需搜索，支持每个连接动态控制
- 📦 **批量操作** - 在单个请求中执行多个工具调用
...
OneMCP 默认**关闭**智能工具发现模式，`tools/list` 会直接返回全部工具列表。启用后，`tools/list` 只返回一个搜索工具（`search_tools`），AI Agent 通过该工具按需搜索所需工具，适合工具数量多的场景。

#### 开启智能工具发现

**CLI 模式**：使用 `--smart-discovery` 参数

```bash
onemcp --smart-discovery
# 或使用 npx
npx onemcp --smart-discovery
```

**Server 模式**：

- **启动时开启**（对所有客户端生效）：使用 `--smart-discovery` 参数

```bash
onemcp --mode server --port 3000 --smart-discovery
```
...
| 低 | `--smart-discovery` 启动参数 | 服务器默认值 |

> **注意**：
>
> - `X-MCP-Smart-Discovery` 支持值：`true`/`1`/`on` 开启，`false`/`0`/`off` 关闭
> - 不传该头时，使用服务器启动时的默认值（未传 `--smart-discovery` 时默认关闭）
...
- **T14 长描述滚动**：Ctrl+E 展开后 `↑/↓` 逐行滚动描述（不再切换工具），滚到尽头后继续按才切换工具
