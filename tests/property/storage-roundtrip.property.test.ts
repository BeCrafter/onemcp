...
/**
 * Feature: onemcp-router-system, Property 2: Configuration persistence round-trip
 * 
...
describe('Feature: onemcp-router-system, Property 2: Configuration persistence round-trip', () => {
  ...
  describe('FileStorageAdapter', () => {
    ...
    beforeEach(async () => {
      ...
      tempDir = path.join(os.tmpdir(), `storage-test-${Date.now()}-${Math.random().toString(36).substring(7)}`);
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (config) => {
            ...
            const iterTempDir = path.join(os.tmpdir(), `storage-test-${Date.now()}-${Math.random().toString(36).substring(7)}`);
          ...
          async (key, values) => {
            ...
            await Promise.all(
              values.map((value, index) => 
                storage.write(`${key}-${index}`, JSON.stringify(value))
              )
            );
