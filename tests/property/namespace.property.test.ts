...
describe('Feature: onemcp-router-system, Property 6: Namespace round-trip', () => {
  const manager = new NamespaceManager();
    ...
    fc.assert(
      fc.property(
        ...
        fc.integer({ min: 2, max: 10 }).chain(n =>
          fc.array(fc.string({ minLength: 1, maxLength: 10 }).filter(s => s.trim().length > 0), { minLength: n, maxLength: n })
            .map(parts => parts.join('__'))
        ),
        (serviceName, toolName) => {
          // Generate namespaced name
